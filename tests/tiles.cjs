...
for(const day of ['2026-09-18','2026-09-19']){const html=c.eventTile(l,tickets,day);assert.ok(html.includes('チケットURL'));assert.ok(html.includes('投げ数'));assert.ok(!html.includes('全体あまり数'));assert.ok(!html.includes('スプシURL'))}
const after=c.eventTile(l,tickets,'2026-09-20');assert.ok(after.includes('全体あまり数</dt><dd>2枚'));assert.ok(after.includes('スプシURL'));assert.ok(!after.includes('チケットURL'));assert.ok(!after.includes('投げ数'));
